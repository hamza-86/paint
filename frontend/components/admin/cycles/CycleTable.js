                ...
                <tr key={cycle.id} className="hover:bg-slate-50/60 transition-colors group">
                  ...
                  <td className="py-4 pl-6 pr-3 whitespace-nowrap">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${isActive ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                      <div>
                        <Link href={`/admin/cycles/${cycle.id}`} className="font-semibold text-slate-900 hover:text-blue-600 transition-colors">
                          {formatDate(cycle.startDate)} → {formatDate(cycle.endDate)}
                        </Link>
                        <div className="text-xs text-slate-400 font-mono mt-0.5">{cycle.id?.slice(0, 12)}…</div>
                      </div>
                    </div>
                  </td>
