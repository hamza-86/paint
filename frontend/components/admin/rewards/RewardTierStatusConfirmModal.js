...
export default function RewardTierStatusConfirmModal({
  isOpen,
  onClose,
  onConfirm,
  tier,
    ...
    >
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-100 p-6 overflow-hidden">
        ...
        <h3 className="text-base font-bold text-slate-900 mb-1">
          {isDeactivate ? 'Deactivate Reward Tier?' : 'Reactivate Reward Tier?'}
        </h3>
        ...
        <p className="text-xs text-slate-500 leading-relaxed mb-4">
          {isDeactivate ? (
            <>
              Deactivating <strong className="text-slate-800">&ldquo;{tier.suggestedRewardName}&rdquo;</strong> ({tier.minPoints}–{tier.maxPoints} pts) will remove it from active reward eligibility calculations. The tier remains preserved in your database.
            </>
          ) : (
            <>
              Reactivating <strong className="text-slate-800">&ldquo;{tier.suggestedRewardName}&rdquo;</strong> ({tier.minPoints}–{tier.maxPoints} pts) will make it available again for painter reward suggestions.
            </>
          )}
        </p>
